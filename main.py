...
if uploaded_file is not None:
    ...
    if st.sidebar.button('Analyze'):
        ...
        # finding the buisisest person in the group(Group Level)
        if selected_user == 'Overall':
            ...
            with col1:
                ax.bar(x.index, x.values, align='center',color='red')
                plt.xticks(rotation='vertical')
                st.pyplot(fig)
        ...
        fig,ax=plt.subplots()
        ax.bar(most_common_df[0],most_common_df[1],align='center',color='red')
        plt.xticks(rotation='vertical')
